...
public class Messages extends ListenerAdapter {
  ...
  public void onGuildMessageReceived(@Nonnull GuildMessageReceivedEvent event) {
    try {
      SQLHandler.MySQLUseDataManager.setMessageReceive(
        timeStamp,
        event.getChannel().getId(),
        event.getMessage().getContentDisplay().length(),
        event.getAuthor().isBot(),
        event.getMember().getRoles().size(),
  ...
  public void onGuildMessageUpdate(@Nonnull GuildMessageUpdateEvent event) {
    try {
      SQLHandler.MySQLUseDataManager.setMessageReceive(
        ...
        event.getMessage().getContentDisplay().length(),
        event.getAuthor().isBot(),
        event.getAuthor().getJDA().getRoles().size(),
        event.getGuild().getId()
      );
    } catch (SQLException throwables) {
      throwables.printStackTrace();
  ...
  public void onGuildMessageReactionAdd(
    ...
    try {
      SQLHandler.MySQLUseDataManager.setMessageReact(
        timeStamp,
        event.getChannel().getId(),
        event.getUser().isBot(),
        event.getUser().getJDA().getRoles().size(),
        event.getGuild().getId(),
